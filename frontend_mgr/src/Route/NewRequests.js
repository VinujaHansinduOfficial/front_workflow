...
const NewRequests = () => {
    ...
    const fetchTemplates = async () => {
      try {
        const response = await fetch(
          "http://localhost:8080/api/template/getall"
        );
  ...
  const fetchWorkflowDetails = async (formId) => {
    try {
      const response = await fetch(`http://localhost:8080/api/workflows/${formId}`);
  ...
  const getFlowNames = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/employee/workflow/${createdBy}`
      );
  ...
  const postWorkProgress = async (attachmentId, uniqueId) => {
    ...
    try {
      for (const [index, item] of Hierarchy.entries()) {
        ...
        const response = await fetch("http://localhost:8080/api/workprogress", {
          method: "POST",
          headers: {
